...
class CustomerRepository extends QueryRepository
{
    ...
    public function editCustomer(array $data)
    {
        ...
        try {
            ...
            $stmt = $this->update("clientes", "numero_cliente = {$data['numero_cliente']}, nome_cliente = {$data['nome_cliente']}, contato_cliente = {$data['contato_cliente']}, cnpj_cliente = {$data['cnpj_cliente']}, gm_cliente = {$data['gm_cliente']}", "unique_id = ".$data['unique_id']." ");
